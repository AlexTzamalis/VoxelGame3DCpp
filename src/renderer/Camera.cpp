#include "renderer/Camera.hpp"
#include "core/Config.hpp"
#include <glm/gtc/matrix_transform.hpp>
#include <algorithm>
#include <cmath>

Camera::Camera(glm::vec3 position, float yaw, float pitch)
    : position_(position), yaw_(yaw), pitch_(pitch), worldUp_(0.0f, 1.0f, 0.0f) {
    currentZoomFov_ = Config::cameraFov;
    updateVectors();
}

glm::mat4 Camera::viewMatrix() const {
    return glm::lookAt(position_, position_ + front_, up_);
}

float Camera::getEffectiveFov() const {
    return currentZoomFov_ + sprintFovOffset_;
}

glm::mat4 Camera::projectionMatrix() const {
    return glm::perspective(glm::radians(getEffectiveFov()), aspect_, near_, far_);
}

void Camera::jump(float velocity) {
    if (isGrounded_) {
        physicsVelocity_.y = velocity;
        isGrounded_ = false;
    }
}

void Camera::adjustZoom(float scrollDelta) {
    zoomTargetFov_ -= scrollDelta * 3.0f;
    zoomTargetFov_ = std::clamp(zoomTargetFov_, 15.0f, 60.0f);
}

void Camera::applyPhysics(float deltaTime, const std::function<bool(glm::vec3, glm::vec3)>& checkCollisionFunc) {
    // === GRAVITY / VERTICAL PHYSICS ===
    if (Config::currentMode == GameMode::SURVIVAL) {
        physicsVelocity_.y -= 32.0f * deltaTime;
        if (physicsVelocity_.y < -50.0f) physicsVelocity_.y = -50.0f;
    } else {
        // Flying modes: no gravity, vertical handled by flyVelocity_
        physicsVelocity_.y = 0.0f;
    }

    // === SMOOTH FLYING (Creative / Spectator) ===
    glm::vec3 moveVelocity;
    if (Config::currentMode != GameMode::SURVIVAL) {
        // Smoothly interpolate fly velocity toward input target
        // This creates gradual acceleration AND deceleration
        float smoothFactor = 1.0f - std::pow(0.001f, deltaTime); // ~99.9% per second convergence
        
        if (glm::length(inputVelocity_) > 0.01f) {
            // Player is pressing keys: accelerate toward target speed
            flyVelocity_ += (inputVelocity_ - flyVelocity_) * smoothFactor;
        } else {
            // No input: decelerate smoothly to zero
            flyVelocity_ *= (1.0f - smoothFactor * 0.7f);
        }
        
        // Kill tiny residual drift
        if (glm::length(flyVelocity_) < 0.02f) flyVelocity_ = glm::vec3(0.0f);
        
        moveVelocity = flyVelocity_;
    } else {
        // Survival: instant response (standard voxel feel)
        moveVelocity = inputVelocity_ + physicsVelocity_;
        flyVelocity_ = glm::vec3(0.0f);
    }

    glm::vec3 moveDelta = moveVelocity * deltaTime;

    // Player base position (feet)
    glm::vec3 playerBase = position_;
    playerBase.y -= eyeOffsetY_;

    // === AXIS-SEPARATED COLLISION ===
    auto collides = [&](glm::vec3 pos) -> bool {
        if (Config::currentMode == GameMode::SPECTATOR) return false;
        glm::vec3 minB = pos - boundingBoxHalfExtents_;
        glm::vec3 maxB = pos + boundingBoxHalfExtents_;
        return checkCollisionFunc(minB, maxB);
    };

    // X-Axis
    playerBase.x += moveDelta.x;
    if (collides(playerBase)) {
        playerBase.x -= moveDelta.x;
        inputVelocity_.x = 0;
        flyVelocity_.x = 0;
    }

    // Z-Axis
    playerBase.z += moveDelta.z;
    if (collides(playerBase)) {
        playerBase.z -= moveDelta.z;
        inputVelocity_.z = 0;
        flyVelocity_.z = 0;
    }

    // Y-Axis
    playerBase.y += moveDelta.y;
    isGrounded_ = false;
    if (collides(playerBase)) {
        playerBase.y -= moveDelta.y;
        if (moveDelta.y < 0) isGrounded_ = true;
        physicsVelocity_.y = 0;
        flyVelocity_.y = 0;
    }

    // Finalize position (back to eye level)
    position_ = playerBase;
    position_.y += eyeOffsetY_;

    // Reset frame input
    inputVelocity_ = glm::vec3(0.0f);

    // === ZOOM FOV INTERPOLATION ===
    float targetFov = isZooming_ ? zoomTargetFov_ : Config::cameraFov;
    float lerpSpeed = 10.0f;
    currentZoomFov_ += (targetFov - currentZoomFov_) * std::min(lerpSpeed * deltaTime, 1.0f);

    // === SPRINT FOV BOOST ===
    float targetSprintFov = isSprinting_ ? 8.0f : 0.0f; // +8° when sprinting
    float sprintLerp = 6.0f;
    sprintFovOffset_ += (targetSprintFov - sprintFovOffset_) * std::min(sprintLerp * deltaTime, 1.0f);
}

void Camera::rotate(float deltaYaw, float deltaPitch) {
    yaw_ += deltaYaw;
    pitch_ += deltaPitch;
    pitch_ = std::clamp(pitch_, -89.0f, 89.0f);
    updateVectors();
}

void Camera::updateVectors() {
    glm::vec3 dir;
    dir.x = cos(glm::radians(yaw_)) * cos(glm::radians(pitch_));
    dir.y = sin(glm::radians(pitch_));
    dir.z = sin(glm::radians(yaw_)) * cos(glm::radians(pitch_));
    front_ = glm::normalize(dir);
    right_ = glm::normalize(glm::cross(front_, worldUp_));
    up_ = glm::normalize(glm::cross(right_, front_));
}

void Camera::updateFrustum() {
    glm::mat4 viewProj = projectionMatrix() * viewMatrix();

    // Left
    frustum_.planes[0].normal.x = viewProj[0][3] + viewProj[0][0];
    frustum_.planes[0].normal.y = viewProj[1][3] + viewProj[1][0];
    frustum_.planes[0].normal.z = viewProj[2][3] + viewProj[2][0];
    frustum_.planes[0].distance = viewProj[3][3] + viewProj[3][0];

    // Right
    frustum_.planes[1].normal.x = viewProj[0][3] - viewProj[0][0];
    frustum_.planes[1].normal.y = viewProj[1][3] - viewProj[1][0];
    frustum_.planes[1].normal.z = viewProj[2][3] - viewProj[2][0];
    frustum_.planes[1].distance = viewProj[3][3] - viewProj[3][0];

    // Bottom
    frustum_.planes[2].normal.x = viewProj[0][3] + viewProj[0][1];
    frustum_.planes[2].normal.y = viewProj[1][3] + viewProj[1][1];
    frustum_.planes[2].normal.z = viewProj[2][3] + viewProj[2][1];
    frustum_.planes[2].distance = viewProj[3][3] + viewProj[3][1];

    // Top
    frustum_.planes[3].normal.x = viewProj[0][3] - viewProj[0][1];
    frustum_.planes[3].normal.y = viewProj[1][3] - viewProj[1][1];
    frustum_.planes[3].normal.z = viewProj[2][3] - viewProj[2][1];
    frustum_.planes[3].distance = viewProj[3][3] - viewProj[3][1];

    // Near
    frustum_.planes[4].normal.x = viewProj[0][3] + viewProj[0][2];
    frustum_.planes[4].normal.y = viewProj[1][3] + viewProj[1][2];
    frustum_.planes[4].normal.z = viewProj[2][3] + viewProj[2][2];
    frustum_.planes[4].distance = viewProj[3][3] + viewProj[3][2];

    // Far
    frustum_.planes[5].normal.x = viewProj[0][3] - viewProj[0][2];
    frustum_.planes[5].normal.y = viewProj[1][3] - viewProj[1][2];
    frustum_.planes[5].normal.z = viewProj[2][3] - viewProj[2][2];
    frustum_.planes[5].distance = viewProj[3][3] - viewProj[3][2];

    for (int i = 0; i < 6; i++) {
        float length = glm::length(frustum_.planes[i].normal);
        frustum_.planes[i].normal /= length;
        frustum_.planes[i].distance /= length;
    }
}

bool Camera::isBoxInFrustum(const glm::vec3& min, const glm::vec3& max) const {
    for (int i = 0; i < 6; i++) {
        glm::vec3 p = min;
        if (frustum_.planes[i].normal.x >= 0) p.x = max.x;
        if (frustum_.planes[i].normal.y >= 0) p.y = max.y;
        if (frustum_.planes[i].normal.z >= 0) p.z = max.z;

        if (glm::dot(frustum_.planes[i].normal, p) + frustum_.planes[i].distance < 0) {
            return false;
        }
    }
    return true;
}
